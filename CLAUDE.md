...
- `emu68-common` is the shared support library for the stack: debug output, GPIO helpers, the DMA-memory facility, a reset guard, byteorder/MMIO accessors, timing, devicetree wrappers, minlist/string/format helpers, and the local helper surface.
- Builds to the static library `libcommon.a` (CMake target `common`, exported as `Emu68Common::common`).
...
Build through the superbuild's container wrapper — never host `cmake` (the build
...
`EMU68_DEBUG_BACKEND` (cache var, default `pistorm`) selects the debug sink and is
honored here and exported to every consumer via the installed
...
- `serial`  — `RawDoFmt` + `putch` → `debug.lib` `KPutChar` (AmigaOS serial @ 9600).
  Links `debug.lib` (carries a 4-byte `_SysBase` `.bss`) → **not** ROM-able.
- `off`     — logging compiled out.

On top of the sink, `EMU68_TIER` (`off` | `profile` | `debug` | `trace`, default
`debug`) selects what is emitted — a cumulative ladder of `PROFILE` ⊂ `DEBUG` ⊂
`TRACE`, one printer each (`KprintfP` / `Kprintf` / `KprintfT`). Backend `off`
defines none of them. `DEBUG_SINK` marks "a sink exists" and is NOT a tier: it
gates the formatter, the tier macros gate the printers, which is what keeps
`perf.c`'s reporter linkable from a higher-tier consumer.

The module exports the functions every debug-emitting component calls instead of
hardcoding `-DDEBUG` / `emu68_rom_check`:
- `emu68_debug_definitions()` — sets `DEBUG_SINK`/`DEBUG_SERIAL` + the tier macros.
- `emu68_tier_at_least(<out> <rung>)` — TRUE when this component's tier reaches
  `<rung>`; for wiring third-party switches (nvme.device does this for the mounter
  submodule's own log/trace defines).
- `emu68_debug_backend_finalize(<tgt> [ROMABLE])` — serial: link `-ldebug` + the
  weak `__divsi3` glue (`cmake/emu68_debug_serial_glue.c`); else: run the ROM check
  for `ROMABLE` (freestanding) targets.

Note: `debug.h` includes `<proto/exec.h>` only under `DEBUG_SINK`; sources that call
exec functions must include it themselves (with the
`#define __NOLIBBASE__` / `EXEC_BASE_NAME (*(struct ExecBase**)4UL)` idiom).

## Key Headers

| Header | Purpose |
|---|---|
| `debug.h` | `KprintfP`/`Kprintf`/`KprintfT`/`KASSERT`/`PrintPistorm` — one printer per tier over a `RawDoFmt` whose per-byte `putch` sink is set by the backend (`pistorm`: magic `0xdeadbeef` PiStorm hook; `serial`: `debug.lib` `KPutChar`); all compiled out in the `off` backend. Sink from `EMU68_DEBUG_BACKEND`, tier from `EMU68_TIER` (see Build Commands) |
